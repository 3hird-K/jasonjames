...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-4 rounded-2xl bg-white/10 px-4 pb-3 md:flex dark:bg-black/20 backdrop-blur-md border border-white/20 dark:border-white/[0.1] shadow-2xl",
        className,
      )}
    >
      {items.map((item) => (
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "relative flex aspect-square items-center justify-center rounded-full bg-white/5 dark:bg-white/[0.02] hover:bg-white/10 dark:hover:bg-white/[0.08] backdrop-blur-sm shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] transition-colors duration-200",
          hovered && "ring-1 ring-white/30 dark:ring-white/[0.2]",
        )}
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="absolute -top-10 left-1/2 w-fit rounded-lg border border-white/20 dark:border-white/[0.1] bg-white/20 dark:bg-black/40 px-3 py-1 text-sm font-medium backdrop-blur-md shadow-xl whitespace-pre text-neutral-800 dark:text-white"
            >
              {title}
            </motion.div>
          )}
